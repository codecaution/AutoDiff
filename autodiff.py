...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        return [matmul_op(output_grad, node.inputs[1] , trans_A=False, trans_B = True),matmul_op(node.inputs[0],output_grad,trans_A=True,trans_B=False)]
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if not isinstance(node.op, PlaceholderOp):
                input_vals = []
                for intput_node in node.inputs:
                    input_vals.append(node_to_val_map[intput_node])
